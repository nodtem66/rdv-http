package org.cardioart.rdv.util
...
object RandomHash {
  ...
  val TOKEN_CHARS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
  ...
  final def generateToken(tokenLength: Int) : String = {
   ...
   *  tokenprifix: string to concatenate with random generated token prior to HASH to improve uniqueness, such as username
     ...
     */
  def generateMD5Token(token: String): String =  {
    md5(token + System.nanoTime() + generateToken(TOKEN_LENGTH))
  }
  def generateSHAToken(token: String): String =  {
    sha(token + System.nanoTime() + generateToken(TOKEN_LENGTH))
  }
}