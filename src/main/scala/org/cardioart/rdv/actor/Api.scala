...
import org.cardioart.rdv.cors.CorsSupportDirectives
...
class Api(connectionSupervisorRef: ActorRef, sessionSupervisorRef: ActorRef)
  extends HttpServiceActor with ActorLogging with CorsSupportDirectives {

  implicit val timeout: Timeout = 1100.millis
    ...
    cors {
      pathSingleSlash {
        ...
        pathPrefix("session") {
          path("start") {
            get {
              parameters('dsn.as[String], 'limit.as[Int] ? 1000, 't.as[Int] ? 15) {
                (dsn, limit, t) => ctx =>
                  val respond = context.actorOf(ResponderActor.props(ctx))
